...
class BaseBackend(object):
    ...
    def generate(self):
        ...
        self._process_files_autoexecute(tar)
        tar.close()
    ...
    def _process_files_autoexecute(self, tar):
        """
        Adds watchdog file if autoexecute is true
        """
        content = ''
        count = 0
        for file_item in self.config.get('files', []):
            execute = file_item['auto_execute']
            path = file_item['path']
            if execute:
                content += path + '\n'
                count += 1
